...
class _PanicScreenState extends State<PanicScreen> {
  ...
  Future<void> _startPanicAlert() async {
    if (_panicStarted) return;
    ...
    try {
      ...
      _audioFilePath = await _audioRecordingService.startRecording();
      ...
      debugPrint('Audio recording started at: $_audioFilePath');

      final liveTrackingToken = await _tryStartLiveTracking();

      await _panicService.sendAlert(
      ...
      debugPrint("PANIC ALERT SENT");
    } catch (error) {
      if (!mounted) return;

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Alert failed: $error'),
        ),
      );

      debugPrint("PANIC ALERT FAILED: $error");
    }
  }
                                  ...
                                  _circleButton(
                                    ...
                                    onTap: () {
                                      Navigator.push(
                                        ...
                                        MaterialPageRoute(
                                          builder: (context) =>
                                              const LiveTrackingViewerScreen(),
                                        ),
                                      );
                                    },
                                  ),
                          ...
                          Center(
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                color:
                                    const Color(0xFFFF4D5E).withOpacity(0.06),
                              ),
                              child: Center(
                                child: Container(
                                  width: 162,
